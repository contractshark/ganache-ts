...
GethApiDouble.prototype.evm_mine_interval = function(seconds, callback) {
  this.state.blockTime = seconds;
  this.state.is_mining_on_interval = !!seconds;

  // This needs some hardening. Split up the mineOnInterval logic
  // so that cancellation logic is decoupled from starting logic.
  if (!this.state.is_mining_on_interval) {
    clearTimeout(this.state.mining_interval_timeout);
    if (this.state._minerCancellationToken !== null) {
      this.state._minerCancellationToken.cancelled = true;
    }
  } else {
    this.state.mineOnInterval();
  }
